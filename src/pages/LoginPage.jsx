import React, { useState } from "react";
import "../styles/auth.css";
import { Link, useNavigate } from "react-router-dom";
import { ClipLoader } from "react-spinners";
import { login } from "../utils/auth";

function LoginPage() {
  const [errors, setErrors] = useState({});
  const [loading, setLoading] = useState(false);

  const [emailValue, setEmailValue] = useState("");
  const [passValue, setPassValue] = useState("");
  const navigate = useNavigate();

  const validateEmail = (email) => {
    if (!email.trim()) {
      return "Email is required.";
    }

    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
      return "Please enter a valid email address.";
    }

    return "";
  };

  const validatePassword = (password) => {
    const trimmedPassword = password.trim();

    if (!trimmedPassword) {
      return "Password is required.";
    }

    return "";
  };

  const submitHandler = async (e) => {
    e.preventDefault();

    const email = emailValue.trim();
    const password = passValue.trim();

    const emailErr = validateEmail(email);
    const passErr = validatePassword(password);

    if (emailErr || passErr) {
      setErrors({
        emailErr,
        passErr,
      });

      return;
    }

    setErrors({});
    setLoading(true);

    try {
      await login(email, password);
      navigate("/chat/new");
    } catch (err) {
      const newErrors = {};

      switch (err.code) {
        case "auth/invalid-email":
          newErrors.emailErr = "Invalid email address.";
          break;

        case "auth/missing-password":
          newErrors.passErr = "Password is required.";
          break;

        case "auth/invalid-credential":
          newErrors.generalErr = "Incorrect email or password.";
          break;

        case "auth/user-disabled":
          newErrors.generalErr = "This account has been disabled.";
          break;

        case "auth/too-many-requests":
          newErrors.generalErr =
            "Too many failed attempts. Please try again later.";
          break;

        case "auth/network-request-failed":
          newErrors.generalErr =
            "Network error. Check your internet connection.";
          break;

        case "auth/internal-error":
          newErrors.generalErr =
            "An unexpected error occurred. Please try again.";
          break;

        default:
          newErrors.generalErr = "Failed to sign in. Please try again.";

          console.error(err);
      }

      setErrors(newErrors);
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="auth-page">
      <form onSubmit={submitHandler} className="auth-form">
        <div className="auth-title">
          <div className="auth-title-dot"></div>
          <h2>Login</h2>
        </div>

        <div className="general-err-container">{errors.generalErr}</div>

        <div className="auth-inputs">
          <div className="input-container">
            <label htmlFor="email" className="mono">
              Email
            </label>

            <input
              type="email"
              id="email"
              value={emailValue}
              onChange={(e) => {
                const value = e.target.value;

                setEmailValue(value);

                setErrors((prev) => ({
                  ...prev,
                  emailErr: validateEmail(value),
                  generalErr: "",
                }));
              }}
              placeholder="Enter your email"
              autoComplete="email"
            />

            <span className="input-err mono">{errors.emailErr}</span>
          </div>

          <div className="input-container">
            <label htmlFor="password" className="mono">
              Password
            </label>

            <input
              type="password"
              id="password"
              value={passValue}
              onChange={(e) => {
                const value = e.target.value;

                setPassValue(value);

                setErrors((prev) => ({
                  ...prev,
                  passErr: validatePassword(value),
                  generalErr: "",
                }));
              }}
              placeholder="••••••••"
              autoComplete="current-password"
            />

            <span className="input-err mono">{errors.passErr}</span>
          </div>
        </div>

        <Link to="/signup" className="auth-form-quick-link mono">
          Don't have an account? Signup
        </Link>

        <button type="submit" disabled={loading} className="auth-btn">
          {loading ? <ClipLoader size={15} /> : "Sign in"}
        </button>
      </form>
    </div>
  );
}

export default LoginPage;
